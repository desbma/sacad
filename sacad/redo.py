""" Module inspired by https://github.com/mozilla-releng/redo, but yielding time to sleep instead of sleeping, to use with asyncio. """
...
def retrier(*, max_attempts, sleeptime, max_sleeptime, sleepscale=1.5, jitter=0.2):
  ...
  assert(sleeptime >= 0)
  assert(jitter <= sleeptime)
  assert(sleepscale >= 1)

  cur_sleeptime = sleeptime

  for attempt in range(max_attempts):
    cur_jitter = random.randint(int(-jitter * 1000), int(jitter * 1000)) / 1000
    yield cur_sleeptime + cur_jitter
